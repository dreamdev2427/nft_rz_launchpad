...
export const WalletSection = () => {
  ...
  const _renderConnectButton = useMemo(() => {
    ...
    if (status === WalletStatus.Connecting) {
      return (
        <button className="rounded-lg w-full inline-flex justify-center items-center font-medium cursor-wait ">
          <svg
    ...
    return (
      <button
        className="inline-flex items-center justify-center w-40 h-12 text-black rounded-lg"
        onClick={onClick}
      >
        <buttonData.icon className="flex-shrink-0 w-5 h-5 mr-2 text-black" />
  ...
  return (
    <div className="max-w-sm  ">
      ...
      <div className="inline-flex items-center justify-center w-40 h-12 text-black rounded-lg bg-[#3f0] ">
